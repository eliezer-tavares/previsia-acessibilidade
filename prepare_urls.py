...
logging.basicConfig(
    filename="erros_prepare_urls.log",
    level=logging.INFO,
    ...
    try:
        ...
        for i, url in enumerate(urls, 1):
            ...
            if i in [10, 30, 60, 100] or (i >= 100 and i % 50 == 0):
                print(
                    f"Progresso na validação: {i}/{total_urls} URLs processadas, {len(valid_urls)} válidas até agora."
                )
        print(
            f"Validação concluída: {len(valid_urls)} URLs válidas de {total_urls} totais."
        )
        valid_urls.append(
            "https://univesp.br"
        )  # EN: Add Univesp URL. Why? Ensures inclusion for testing.
        ...
        logging.info(
            f"EN: CSV generated with {len(valid_urls)} URLs: data/tranco_top_10000.csv. PT: CSV gerado com {len(valid_urls)} URLs: data/tranco_top_10000.csv"
        )
        print(f"CSV gerado com sucesso: data/tranco_top_10000.csv")

    except Exception as e:
        logging.error(
            f"EN: Error generating CSV: {str(e)}. PT: Erro ao gerar CSV: {str(e)}."
        )
