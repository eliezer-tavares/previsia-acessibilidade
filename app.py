...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'dev' # TODO: Em produção, use um valor secreto gerado por os.urandom(24)
app.config['BABEL_DEFAULT_LOCALE'] = 'pt_BR'
app.config['BABEL_TRANSLATION_DIRECTORIES'] = 'translations'
app.permanent_session_lifetime = timedelta(days=31)  # FIX: Sessions permanentes pra idioma persistir (31 dias)
...
try:
    ...
    path_modelo = os.path.join(DIRETORIO_MODELO, 'modelo_acessibilidade.pt') # Usar o arquivo .pt
    ...
    logging.error(f"EN: Model artifact not found: {e}. PT: Artefato do modelo não encontrado: {e}.")
...
def gerar_guia_preditivo(features, score, url):
    ...
    """
    guia = _("Análise da página {0} de {1}. Pontuação prevista: {2}. Guia rápido e alertas: ").format(
        url.split('//')[-1].split('/')[0], url, score)
    ...
    if layout.get('nav_itens', 0) > 0:
        guia += ngettext("seguido por um menu de navegação com {0} item.", "seguido por um menu de navegação com {0} itens.", layout['nav_itens']).format(layout['nav_itens'])
        if features.get('pct_links_genericos', 0) > 20:
            guia += _("Cuidado: muitos links genéricos no menu podem confundir a navegação. ")
    if layout.get('carousel_imagens', 0) > 0:
        guia += ngettext("Em seguida, há uma galeria ou carrossel com {0} imagem.", "Em seguida, há uma galeria ou carrossel com {0} imagens.", layout['carousel_imagens']).format(layout['carousel_imagens'])
        if layout.get('carousel_sem_alt', 0) > 0:
            guia += ngettext("Alerta: {0} imagem sem descrição; seu leitor de tela pode ignorá-la.", "Alerta: {0} imagens sem descrição; seu leitor de tela pode ignorá-las.", layout['carousel_sem_alt']).format(layout['carousel_sem_alt'])
    if layout.get('main_presente'):
        guia += _("A área de conteúdo principal segue, ")
        if features.get('erros_hierarquia', 0) > 0:
            guia += ngettext("mas com {0} erro de hierarquia em títulos, o que pode bagunçar a navegação por seções.", "mas com {0} erros de hierarquia em títulos, o que pode bagunçar a navegação por seções.", features['erros_hierarquia']).format(features['erros_hierarquia'])
    if layout.get('form_campos', 0) > 0:
        guia += ngettext("há um formulário com {0} campo no meio ou final da página.", "há um formulário com {0} campos no meio ou final da página.", layout['form_campos']).format(layout['form_campos'])
        if features.get('inputs_sem_label', 0) > 0:
            guia += ngettext("Alerta: {0} campo sem rótulo adequado; formulários podem ser difíceis.", "Alerta: {0} campos sem rótulos adequados; formulários podem ser difíceis.", features['inputs_sem_label']).format(features['inputs_sem_label'])
        else:
    ...
    if features.get('imagens_sem_alt', 0) > 0:
        guia += ngettext("Alerta: {0} imagem sem texto alternativo pode dificultar a navegação para leitores de tela.", "Alerta: {0} imagens sem texto alternativo podem dificultar a navegação para leitores de tela.", features['imagens_sem_alt']).format(features['imagens_sem_alt'])
    if features.get('videos_sem_captions', 0) > 0:
        guia += ngettext("Alerta: {0} vídeo sem legenda; áudio pode ser inacessível.", "Alerta: {0} vídeos sem legendas; áudio pode ser inacessível.", features['videos_sem_captions']).format(features['videos_sem_captions'])
    if features.get('falhas_contraste', 0) > 0:
        guia += ngettext("Alerta: {0} problema de contraste detectado, afetando legibilidade para baixa visão.", "Alerta: {0} problemas de contraste detectados, afetando legibilidade para baixa visão.", features['falhas_contraste']).format(features['falhas_contraste'])
    if features.get('aria_presente', 0) == 0:
        guia += _("Alerta: Falta de ARIA em elementos dinâmicos; interações podem não ser anunciadas. ")
    ...
    if score > 80:
        ...
        guia += _("Sugiro usar ferramentas alternativas como busca do site para evitar frustrações.")
...
def predict():
    if not all([modelo, scaler, feature_names]):
        logging.error("EN: Model or artifacts not available. PT: Modelo ou artefatos não disponíveis.")
        return render_template('resultado.html', error=_("Modelo ou seus componentes não estão disponíveis. Verifique a configuração do servidor."), url="", features={})
    ...
    try:
        if tipo_analise == 'completa':
            ...
            if features is None:
                ...
                aviso = _("A análise completa falhou; usamos a análise rápida como fallback para {0}.").format(url)
        else:
            features = analisar_url_rapida(url)
        if not features:
            logging.error(f"EN: Failed to analyze URL {url}. PT: Falha ao analisar URL {url}.")
            return render_template('resultado.html', error=_("Falha ao extrair características da URL: {0}").format(url), url=url, features={})
        ...
        return render_template('resultado.html', error=_("Ocorreu um erro inesperado during the analysis: {0}").format(e), url=url, features={})
