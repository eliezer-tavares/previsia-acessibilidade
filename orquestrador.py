...
def gera_dataset(batch_size=5874): # Processa todas, mas filtra processadas
    ...
    if partial_files:
        ...
        urls_to_process = [url for url in all_urls if url not in processed_urls][:batch_size]
        print(f"Resumindo de {last_partial}: {len(dados_existentes)} já processadas. Faltam {len(urls_to_process)} novas.")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(analisar_url_completa, url): url for url in urls_to_process}
        for i, future in enumerate(as_completed(futures)):
            url = futures[future]
            print(f"Processando {processed_count + i + 1}/{len(all_urls)} (nova: {i+1}/{len(urls_to_process)}): {url}")
            try:
                ...
                if resultado:
                    ...
                    processed_count += 1
                    print(f"Sucesso! Total sucessos: {processed_count}")  # <-- Adicionado para monitorar sucessos
                    # Save parcial a cada 100 novas processadas com sucesso
                    if (processed_count % 100 == 0):
                        partial_df = pd.DataFrame(dados)
                        cols = ['url', 'label_score_acessibilidade'] + [c for c in partial_df.columns if c not in ['url', 'label_score_acessibilidade']]
                        partial_df = partial_df[cols]
                        partial_filename = f'data/dataset_acessibilidade_partial_{processed_count}.csv'
                        partial_df.to_csv(partial_filename, index=False)
                        print(f"Checkpoint salvo: {processed_count} processadas no total em {partial_filename}")
                else:
